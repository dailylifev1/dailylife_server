...
public class HeartController {
    ...
    @ApiOperation(value = "댓글 좋아요", notes = "commentNum값(tbl_comment)PK값만 넘겨주시면 됩니다. 한 번 누르면 좋아요 / 2번 클릭 시 좋아요 취소")
    @PostMapping("/commentHeartPlus")
    public ResponseEntity<Boolean> commentHeartPlus(@Valid @RequestBody HeartStateRequest heartStateRequest){
    ...
    @ApiOperation(value="대댓글 좋아요", notes = "replyNum(tbl_reply)(대댓글)PK값 넘겨주시면 됩니다. 한 번 누르면 좋아요 / 2번 클릭 시 좋아요 취소")
    ...
    @ApiOperation(value = "댓글 좋아요 개수 출력", notes = "commentNum값(tbl_comment)PK값만 넘겨주시면 됩니다.")
    @GetMapping("/countHeartComment/{commentNum}")
    public ResponseEntity<Long> countHeartReply(@PathVariable("commentNum")Long commentNum){
        System.out.println("댓글 좋아요 총 개수 출력 : "+ heartService.heartCountComment(commentNum));
        return ResponseEntity.ok(heartService.heartCountComment(commentNum));
    ...
    public ResponseEntity<Long> countHeartReplyReply(@PathVariable("replyNum")Long replyNum){
        System.out.println("대댓글 좋아요 총 개수 출력 : "+ heartService.heartCountReply(replyNum));
